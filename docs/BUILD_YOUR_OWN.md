...
A user opens an editor tab, authors one or more timed body-motion prompt beats, chooses seed/guidance, generates one or more candidates, previews the result, edits exact skeletal poses on a timeline, optionally regenerates with those poses as constraints, reopens prior generations without rerunning the model, retargets the chosen motion to a UE5 mannequin or MetaHuman body, and saves a UAnimSequence in the host project.
...
- Pin every source/model repository to an immutable revision and verify the exact runtime
  configurations and weights by byte size and SHA-256 before importing model code.
...
4. An asynchronous HTTP client for health, generate, job polling, persisted history,
   backend capabilities, and shutdown.
...
- GET /history: bounded completed records that can be loaded without regeneration.
- GET /backends: path-free capability and promotion-gate status.
...
- Bound prompt length, segment count, total duration, transition width, diffusion steps,
  candidate count, constraint payload size, retained job history, provenance reads, and
  filesystem destinations.

GENERATION AND AUTHORING DATA

Record the exact ordered prompt/duration sequence, transition width, resolved model,
verified model-bundle digest, backend, seed, candidate seed/index, duration, text
guidance, constraint guidance, text-encoder precision, in-place choice, constraint
preset, exact constraints, output shape, UTC creation time, and artifact paths in
provenance JSON.

Treat the generated output directory as a local history store. Admit only bounded,
schema-valid provenance files and exact non-symlink artifacts contained by that
directory. Restoring history must repopulate generation controls and import an artifact
without submitting a new model job.
...
- Package a model manifest and architecture decisions with the plugin.
...
- Never return configured checkout, executable, weight, cache, or evidence paths from a
...
4. Resolve and verify the pinned model dependency closure, then load LLM2Vec + Llama
   text conditioning and Kimodo in a warm sidecar.
...
9. Add deterministic variations, timed prompt sequences, and candidate selection.
10. Add persisted generation history with path-contained artifact validation.
11. Prove source tests, Unreal automation, C++ build, clean BuildPlugin packaging, fresh-host install, and human viewport acceptance.
...
- A prompt sequence creates deterministic candidate BVH/NPZ/JSON artifacts and a retargeted UAnimSequence.
- Model files fail closed on revision, size, or hash mismatch, and provenance identifies
  the verified bundle.
- A completed generation can be restored after restart without another model call.
...
If investigating a native backend, keep it behind a non-selectable adapter experiment.
Require a clean audited source revision, controlled Windows build, exact skeleton and
constraint compatibility, embedding and motion parity, same-workload latency/VRAM
comparison, and licence review before any production switch.
```
